...
export const Resizeable: React.FC<IProps> = ({
  ...
  useClickOutside($container, {
    in: () => $container.current && $container.current.classList.add("isActive"),
    out: () => $container.current && $container.current.classList.remove("isActive"),
  });
  ...
  return (
    <div
      ...
      className={cls(className, "resizable")}
      ref={$container}
      style={{ ...style, width, height }}
    >
      {isEditable && (
        <>
          <div ref={$placeholderNode} className="placeholderWrap" style={{ opacity: 1 }}>
            <span className={"resizer" + ' ' + "topLeft"} ref={$topLeft} data-type={'topLeft'}></span>
            <span className={"resizer" + ' ' + "topRight"} ref={$topRight} data-type={'topRight'}></span>
            <span
          ...
          <div ref={$cloneNode} className={"cloneNodeWrap"} style={{ width, height, maxWidth }}>
            <span className={"resizer" + ' ' + "topLeft"} ref={$topLeft} data-type={'topLeft'}></span>
            <span className={"resizer" + ' ' + "topRight"} ref={$topRight} data-type={'topRight'}></span>
            <span
              className={"resizer" + ' ' + "bottomLeft"}
